		...
		<form name="myform" method="post" class="was-validated">
			<table class="table table-bordered">
				...
				<tr>
					<th>내용</th>
					<td>
						<textarea rows="6" name="content" id="title" placeholder="내용을 입력해주세요." required class="form-control">${vo.content}</textarea>
						<script>
			...
			<table class="table table-borderless">
				<tr>
					<td colspan="2" class="text-center">
						...
						<input type="button" value="돌아가기" onclick="location.href='${ctp}/board/BoardContent?idx=${vo.idx}&pag=${pVO.pag}&pageSize=${pVO.pageSize}';" class="btn btn-danger" />
					</td>
				</tr>
			</table>
			<input type="hidden" name="mid" value="${sMid}" />
			<input type="hidden" name="idx" value="${vo.idx}" />
			<input type="hidden" name="pag" value="${pVO.pag}" />
			<input type="hidden" name="pageSize" value="${pVO.pageSize}" />
